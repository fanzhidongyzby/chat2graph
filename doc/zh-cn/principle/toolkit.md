...
1. **初始配置**: 系统通过YAML配置预设的 `Action`，`Tool` 集合，以及 `Operator` 绑定的 `Action` 集合。其中图数据库的操作工具已作为内置能力集成到系统内部，只需要通过 [Graph DataBase](../cookbook/graphdb.md) 服务注册即可。另外，动态工具注册能力还在建设当中。
